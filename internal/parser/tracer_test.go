...
func TestTracerMergeWithSideParentIncluded(t *testing.T) {
	...
	for _, cursor := range []int{0, 1} {
		tracer := NewTracer(rows, cursor, 0, len(rows))
		assert.True(t, tracer.IsInSameLane(1))
		assert.True(t, tracer.IsInSameLane(2), "S is a parent of B")
		assert.True(t, tracer.IsInSameLane(3))
		assert.True(t, tracer.IsGutterInLane(1, 1, 0), "fork junction")
		assert.True(t, tracer.IsGutterInLane(1, 1, 1), "side-parent branch")
		assert.True(t, tracer.IsGutterInLane(1, 1, 2), "side-parent corner")
		assert.True(t, tracer.IsGutterInLane(2, 0, 2), "side-parent node")
		assert.Equal(t, "╮", tracer.UpdateGutterText(1, 1, 2, "╮"), "corner must remain connected")
	}
}
